...
3. The owner starts from a generated draft preview across all four process-form areas: challenges/risks/impacts, commitments/targets, management approach, and performance. It uses linked data-point responses, preserves existing owner text, and marks unsupported items for owner input. The owner can insert each section separately or fill only empty sections. The separate standards-review preview flags blank areas and reminds the owner to verify evidence. These are deterministic local prototype drafts, not connected LLM output; attached source files are not parsed. Save retains the red drafting stage; Submit moves the chapter to orange focal-point review and records a mock CSSM notice.
